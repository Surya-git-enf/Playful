...
SUPABASE_URL = os.getenv("SUPABASE_URL", "[https://your-project.supabase.co](https://your-project.supabase.co)")
...
# NOTE: Email removed. User identified securely via JWT.
# ==========================================
...
async def verify_user(credentials: HTTPAuthorizationCredentials = Security(security)) -> dict:
    """Verifies JWT Token with Supabase, prevents IDOR, and handles daily credits rollover."""
    try:
        ...
        user = res.data[0]
        user["email"] = email # Attach email internally
        ...
        if days_passed > 0:
            ...
            updates = {"credits": current_credits, "plan_days": plan_days, "plan": plan, "last_reset_date": str(today)}
...
async def github_api(method: str, endpoint: str, json_data: dict = None, return_status: bool = False):
    ...
    headers = {"Authorization": f"Bearer {PLAYFUL_GH_TOKEN}", "Accept": "application/vnd.github.v3+json"}
    async with httpx.AsyncClient() as client:
        resp = await client.request(method, url, headers=headers, json=json_data, timeout=30.0)
        if return_status: return resp.status_code, resp.json() if resp.text else {}
        if resp.status_code >= 400: raise Exception(f"GitHub API Error: {resp.text}")
...
async def fetch_existing_game_code(username: str, game_name: str) -> str:
    ...
    if status == 200 and "content" in data: return base64.b64decode(data["content"]).decode('utf-8')
    return None
...
async def ensure_user_repo_exists(username: str):
    ...
    if status == 404:
        try: await github_api("POST", f"/orgs/{GITHUB_OWNER}/repos", {"name": username, "auto_init": True})
        except: await github_api("POST", "/user/repos", {"name": username, "auto_init": True})
...
async def delete_folder_from_github(username: str, game_name: str):
    ...
    if status == 200 and isinstance(files, list):
        for file in files:
            await github_api("DELETE", f"/repos/{GITHUB_OWNER}/{username}/contents/{file['path']}", json_data={"message": f"Delete {file['path']}", "sha": file['sha']})
...
async def process_and_upload_assets(job_id: str, username: str, game_name: str, uids: List[str]) -> List[str]:
    ...
    async with httpx.AsyncClient(follow_redirects=True) as client:
        for uid in uids:
            dl_res = await client.get(f"[https://api.sketchfab.com/v3/models/](https://api.sketchfab.com/v3/models/){uid}/download", headers=headers)
            if dl_res.status_code != 200: continue
            zip_url = dl_res.json().get("glb", {}).get("url") or dl_res.json().get("gltf", {}).get("url")
            ...
            with zipfile.ZipFile(io.BytesIO(zip_res.content)) as z:
                ...
                for file_info in z.infolist():
                    ...
                    if clean_name.endswith('.glb') or clean_name.endswith('.gltf'):
                        asset_urls.append(f"[https://raw.githubusercontent.com/](https://raw.githubusercontent.com/){GITHUB_OWNER}/{username}/main/{game_name}/assets/{uid}_{clean_name}")
                if files_to_push: await commit_files_to_github(username, game_name, files_to_push, is_binary=True)
    return asset_urls
...
# ==========================================
# 6. WORKFLOWS (With Custom Gen-Z Messages)
# ==========================================
async def generate_and_commit_workflow(job_id: str, req: GenerateRequest, user: dict):
    try:
        ...
        if current_code: await manager.send_update(job_id, "Remixing Reality 🎛️", "Injecting your brand new mods into the matrix... ⚡🛠️")
        else: await manager.send_update(job_id, "Forging the World 🌎", "Big Bang in progress! Crafting your 3D universe from scratch... 🌌💥")
        ...
        if user["credits"] < cost: raise Exception(f"Insufficient credits. Costs {cost}, you have {user['credits']}.")
        ...
        game_history.extend([{"role": "user", "content": req.prompt, "ts": ts}, {"role": "assistant", "content": manifest["assistant_message"], "ts": ts + 1}])
...
async def build_apk_workflow(job_id: str, req: BuildAPKRequest, user: dict):
    try:
        ...
        if is_free_user: await manager.send_update(job_id, "Initializing", "Booting up the Playful Engine... 🚀", {"progress": 0})
        else: await manager.send_update(job_id, "Initializing", "Verifying Pro License... 👑", {"progress": 0})
        
        if user.get("builds", 0) < 1: raise Exception("Insufficient APK build limits. Please upgrade.")
        build_cost = 5 if is_free_user else 10
        if user.get("credits", 0) < build_cost: raise Exception("Insufficient credits for APK build.")
        ...
        supabase.table("users").update({"builds": user["builds"] - 1, "credits": user["credits"] - build_cost}).eq("id", user["id"]).execute()
        
        dispatch_payload = {
            "ref": "main", "inputs": {
                "owner": GITHUB_OWNER, "repo": user["username"], "folder": req.game_name,
                "admob_banner": final_banner, "admob_interstitial": final_interstitial,
                "ad_interval_minutes": final_ad_interval, "watermark": watermark_enabled
            }
        }
        ...
        if is_free_user: await manager.send_update(job_id, "Watermark", "Applying the Playful Watermark... 💧", {"progress": 35})
        else: await manager.send_update(job_id, "Watermark", "Stripping all watermarks for Pro Build... 🚫💧", {"progress": 35})
        await asyncio.sleep(10)
        if is_free_user: await manager.send_update(job_id, "Monetization", "Wiring up the Playful Ad Network... 💸", {"progress": 50})
        else: await manager.send_update(job_id, "Monetization", "Injecting YOUR custom AdMob IDs... 💰", {"progress": 50})
        await asyncio.sleep(15)
        await manager.send_update(job_id, "Capacitor", "Forging the native Android shell... 🛡️", {"progress": 65})
        await asyncio.sleep(20)
        if is_free_user: await manager.send_update(job_id, "Compiling", "Compiling Gradle code (Grab a coffee, this takes a minute ☕)...", {"progress": 80})
        else: await manager.send_update(job_id, "Compiling", "Compiling High-Speed Native Code... ⚡", {"progress": 80})
        await asyncio.sleep(40)
        if is_free_user: await manager.send_update(job_id, "Finalizing", "Signing and polishing the final APK... ✨", {"progress": 95})
        else: await manager.send_update(job_id, "Finalizing", "Signing your custom App Bundle... ✨", {"progress": 95})
        await asyncio.sleep(15) 
        ...
        if is_free_user: await manager.send_update(job_id, "Build Complete!", "Level Unlocked! Your Android game is ready! 🎮", {"progress": 100, "apk_url": apk_url})
        else: await manager.send_update(job_id, "Build Complete!", "Masterpiece Complete! Your game is ready for the Play Store! 🏆", {"progress": 100, "apk_url": apk_url})
...
async def api_search_assets(request: Request, req: AssetSearchRequest, user: dict = Depends(verify_user)):
    ...
    except: keywords = ["character", "environment"]

    results = {}
    async with httpx.AsyncClient() as client:
        for keyword in keywords[:3]:
            params = {"type": "models", "downloadable": "true", "license": "cc0", "q": keyword, "sort_by": "-relevance"}
            res = await client.get("[https://api.sketchfab.com/v3/search](https://api.sketchfab.com/v3/search)", params=params)
            if res.status_code == 200:
                models = []
                for item in res.json().get("results", [])[:5]:
                    models.append({"name": item.get("name"), "uid": item.get("uid"), "thumbnail": item.get("thumbnails", {}).get("images", [{}])[0].get("url", "")})
...
async def api_add_admob(request: Request, req: AddAdmobRequest, user: dict = Depends(verify_user)):
    if user.get("plan", "free") == "free": raise HTTPException(status_code=403, detail="AdMob integration requires Creator or Studio plan.")
    try:
        supabase.table("users").update({"admob_banner": req.admob_banner, "admob_interstitial": req.admob_interstitial, "admob_interval": req.admob_interval}).eq("id", user["id"]).execute()
...
async def api_edit_game_name(request: Request, req: EditGameNameRequest, user: dict = Depends(verify_user)):
    ...
    try:
        ...
        if not old_folder_sha: raise Exception("Original game folder not found on GitHub.")

        tree_items = [{"path": req.old_game_name, "mode": "040000", "type": "tree", "sha": None}, {"path": req.new_game_name, "mode": "040000", "type": "tree", "sha": old_folder_sha}]
...
async def api_get_games(request: Request, user: dict = Depends(verify_user)):
    ...
    return {"games": [{"game_name": i["name"], "preview_url": f"https://{GITHUB_OWNER}.github.io/{user['username']}/{i['name']}/index.html", "last_updated": "In Repo"} for i in contents if i["type"] == "dir"]}

@app.post("/getchat")
@limiter.limit("30/minute")
async def api_get_chat(request: Request, req: GameRequest, user: dict = Depends(verify_user)):
    return {"game_name": req.game_name, "chat": user.get("chat_history", {}).get(req.game_name, [])}
...
async def websocket_endpoint(websocket: WebSocket, job_id: str):
    ...
    try:
        if job_id in JOB_STORE: await websocket.send_json({"job_id": job_id, **JOB_STORE[job_id]})
        while True: await websocket.receive_text()
    except WebSocketDisconnect: manager.disconnect(job_id)
